...
public class 뱀술 {
	...
	static class Snake {
		...
		private boolean isAlive(int oldX, int oldY, int newX, int newY) {
			...
			// 직선을 위한 크기 비교
			int newBigX = oldX > newX ? oldX : newX;
			int newSmallX = oldX > newX ? newX : oldX;
			
			int newBigY = oldY > newY ? oldY : newY;
			int newSmallY = oldY > newY ? newY : oldY;
			
			// 직전 경로를 제외하고 이제까지의 경로들을 직선 형태로 비교
			for(int i=0; i<moveInfos.size()-1; i++) {
				...
				// 둘다 세로직선인 경우 
				if(oldBigX == oldSmallX && newBigX == newSmallX) {
					
					// X좌표가 같다면 y좌표를 체크해줘야한다.
					if(oldBigX == newBigX) {
						
						if(dy == 1) {
							if(newSmallY <= oldSmallY && newBigY >=oldSmallY) {
								tempAliveTime = oldSmallY - newSmallY;
								dieTime = Math.min(dieTime, tempAliveTime);
							}
						}
						else if(dy == -1) {
							if(newBigY >= oldBigY && newSmallY <= oldBigY ) {
								tempAliveTime = newBigY - oldBigY;
								dieTime = Math.min(dieTime, tempAliveTime);
							}
						}
				...
				// 둘다 가로 직선인 경우
				else if(oldBigY == oldSmallY && newBigY == newSmallY) {
					// Y좌표가 같다면 x좌표를 체크해줘야한다.
					if(oldBigY == newBigY) {
						// 충돌하는 경우
						if(dx == 1) {
							if(newSmallX <= oldSmallX && newBigX >= oldSmallX) {
								tempAliveTime = oldSmallX - newSmallX;
								dieTime = Math.min(dieTime, tempAliveTime);
							}
						} 
						else if (dx == -1){
							if(newBigX >= oldBigX && newSmallX <= oldBigX ) {
								tempAliveTime = newBigX - oldBigX;
								dieTime = Math.min(dieTime, tempAliveTime);
							}
						}
					}
				...
				// 과거 경로가 세로직선, 새 경로가 가로 직선인 경우 (bigY == smallY)
					...
					if(newSmallX <= oldBigX && oldBigX <= newBigX) {
						if(oldSmallY <= newBigY && newBigY <= oldBigY) {
							// 사망 return
							if(dx == 1) {
				...
				// 과거 경로가 가로직선, 새 경로가 세로 직선인 경우 (bigX == smallX)
					...
					if(newSmallY <= oldBigY && oldBigY <= newBigY) {
						if(oldSmallX <= newBigX && newBigX <= oldBigX) {
